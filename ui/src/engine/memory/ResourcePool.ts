...
import { BufferGeometry, Material, Mesh, Vector3, Quaternion, Matrix4, Color } from 'three';
...
export class MaterialPool {
  private static pools: Map<string, ObjectPool<Material>> = new Map();

  static getMaterial<T extends Material>(
    key: string,
    factory: () => T,
    resetFn?: (material: T) => void
  ): T {
    if (!this.pools.has(key)) {
      this.pools.set(key, new ObjectPool<Material>(factory as () => Material, resetFn as (obj: Material) => void));
    }
    return this.pools.get(key)!.get() as T;
  }

  static releaseMaterial(key: string, material: Material): void {
...
export class MeshPool {
  private static pools: Map<string, ObjectPool<Mesh>> = new Map();

  static getMesh(
    key: string,
    geometryFactory: () => BufferGeometry,
    ...
    if (!this.pools.has(key)) {
      ...
      this.pools.set(key, new ObjectPool<Mesh>(factory as () => Mesh, resetFn as (obj: Mesh) => void, 20));
    }
    
    return this.pools.get(key)!.get();
  }

  static releaseMesh(key: string, mesh: Mesh): void {
    const pool = this.pools.get(key);
    if (pool) {
      pool.release(mesh);
    }
...
if (typeof window !== 'undefined') {
  ...
  if (process.env.NODE_ENV === 'development') {
    poolStatsInterval = setInterval(() => {
      ...
      if (hasContent && import.meta.env.DEV) {
