...
export class AnimationManager {
  ...
  requestAnimationFrame(
    ...
    // Use a wrapper object to hold the animation frame ID
    const handleWrapper: { rafId?: number } = {};
    ...
    this.handles.set(id, {
      ...
      cleanup: () => {
        if (handleWrapper.rafId) {
          cancelAnimationFrame(handleWrapper.rafId);
        }
        this.handles.delete(id);
      },
    });
  ...
  cancelType(
    ...
    const toCancel = Array.from(this.handles.values()).filter(
      (h) => h.type === type,
    );
