...
- `Features/Notifications/` — `GetNotifications` (unread counts for the bell badge) and `MarkNotificationsRead`. Counts are **derived**, not stored: a `NotificationState` document per user in the `users` container holds `postsSeenAt`/`commentsSeenAt` watermarks, and the handler counts posts/replies/likes newer than those. Nothing is written when an event happens — only when the user marks them read. First read seeds the watermarks at "now" so a new user doesn't see every post ever published.
